...
client = MongoClient("mongodb://admin:neepodapati@149.165.155.201:27017/admin")

db = client["INT_records"]

collection = db["bursts"]
...
def vlan_hour():
    ...
    print(request.args.get("from")+" looooooooook "+request.args.get("to"))
    vlan_pipeline(start,end,interval)
    ...
    results = collection.aggregate(vlan_vs_hour)
...
def key_hour():
    ...
    results = collection.aggregate(key_vs_hour)
    data = list(collection.find({}, {"_id": 0}))
    return jsonify(list(results))
